...
<html lang="ja">
    ...
    <body style="font-family: 'Roboto Mono', 'Noto Sans JP', sans-serif;">
        ...
        <form class="flex flex-col w-5/6 mx-auto mb-20" action="/diaries/{{ $diary->id }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method("PUT")
            <div class="space-y-8">
                        ...
                        @foreach($weathers as $weather)
                            <label class="cursor-pointer mx-4" for="weather-{{ $weather->id }}">
                                <input class="hidden peer" type="radio" name="diary[weather_id]" id="weather-{{ $weather->id }}" value="{{ $weather->id }}"
                                    {{ old('diary.weather_id', $selectedWeather) == $weather->id ? 'checked' : ''}}>
                ...
                <div class="title space-y-4">
                    ...
                    <input class="min-w-96 focus:border-black focus:ring-0" type="text" name="diary[title]" value="{{ old('diary.title', $diary->title) }}"/>
                    <p class="title_error" style="color:red">{{ $errors->first('diary.title') }}</p>
                </div>
                <div class="body space-y-4">
                    <p>Body.</p>
                    <textarea id="textarea" class="w-1/2 min-w-96 focus:border-black focus:ring-0 resize-none overflow-hidden" name="diary[body]">{{ old('diary.body', $diary->body) }}</textarea>
                    ...
                    <div class="tags items-center flex flex-wrap">
                        @foreach($tags as $tag)
                            <div class="py-2">
                                ...
                                -->
                                    {{ $tag->name }}<input class="hidden" type="checkbox" name="existing_tag_ids[]" id="tag-{{ $tag->id }}" value="{{ $tag->id }}"
        ...
        <script>
            ...
            document.addEventListener('DOMContentLoaded', function () {
                ...
                function updateWeekday(){
                    ...
                    // 要素の存在を確認
                    console.log(yearSelect, monthSelect, dateSelect);
                    
                    const selectedDay = new Date(selectedYear, selectedMonth, selectedDate).getDay();
                    const weekdays = ['sun','mon','tue','wed','thu','fri','sat'];
                    
                    // console.logで曜日とID=weekdayの要素を確認
                    console.log('選択された日:', selectedDay, '曜日:', weekdays[selectedDay]);
                    console.log('ID=weekdayの要素:', document.getElementById('weekday'));
